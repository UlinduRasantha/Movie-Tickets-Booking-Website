...
import { ArrowRightIcon, ClockIcon, Divide, User } from 'lucide-react'
...
const SeatLayout = () => {
  ...
  const bookTickets = async () => {
    try {
      if(!User) return toast.error('Please login to proceed')
        ...
        if(!setSelectedTime || !selectedSeats.length) return toast.error
  ...
  return show ? (
    ...
    md:pt-50'>
      ...
      h-max md:sticky md:top-30'>
        ...
        <div className='mt-5 space-y-1'>
          {show.dateTime[date].map((item) => (
            <div key={item.time} onClick={() => setSelectedTime(item)} className=
            {`flex items-center gap-2 px-6 py-2 w-max rounded-r-md
      ...
      {/* Seats Layout */}
      <div className='relative flex-1 flex flex-col items-center max:mt-16'>
          ...
          <p className='text-gray-400 text-sm mb-6'>SCREEN SIDE</p>
            ...
            bg-primary hover:bg-primary-dull transition rounded-ful font-medium
