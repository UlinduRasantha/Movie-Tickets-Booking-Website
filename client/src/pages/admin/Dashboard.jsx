...
const Dashboard = () => {
  ...
  const dashboardCasrd = [
    {title: "Total Bookings", value: dashbordData.totalBookings || "0",
       icon:ChartLineIcon},
    {title: "Total Revenue", value: currency + dashbordData.totalRevenue || "0",
       icon:CircleDollarSignIcon},
    {title: "Active Shows", value: dashbordData.activeShows.length || "0",
       icon:PlayCircleIcon},
    {title: "Total Bookings", value: dashbordData.totalUser || "0",
       icon:UserIcon },
  ...
  const fetchDashbordData = async () => {
    try {
      const { data } = await axios.get("/api/admin/dashboard", {
        ...
        if(data.success){
          setDashbordData(data.dashbordData)
          setLoading(false)
        }else{
          toast.error(data.message)
        }
    } catch (error) {
      toast.error("Error featching dashboard data:", error)
    }
  };
  ...
  return  !loading ?(
    <>
      ...
      <p className='mt-10 text--lg font-medium'>Action Shows</p>
