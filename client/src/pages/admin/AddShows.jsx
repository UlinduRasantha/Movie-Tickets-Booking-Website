...
const AddShows = () => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      if(!seletedMovie || Object.keys(dateTimeSelection).length === 0 || 
      !showPrice){
  ...
  return nowPlayingMovies.length > 0 ? (
    <>
      ...
      <div className='overflow-x-auto pb-4 no-scrollbar'>
        <div className='group flex flex-wrap gap-4 mt-4 w-max '>
          {nowPlayingMovies.map((movie)=>(
            ...
            duration-300`} onClick={()=> setSeletedMovie(movie.id)}>
      ...
      <div className='mt-6'>
        ...
        rounded-lg'>
          <input type="datetime-local" value={dateTimeInputs}  onChange=
          {(e)=> setDateTimeInputs(e.target.value)}
